...
fun main() {
    ...
    while (true) {
        ...
        when (command.getCurrent()) {
            ...
            in sets.keys -> makeOperation(command.split(" "))
...
fun makeOperation(data: List<String>) {
    if (data.size != 3)
        errorMessage("wrong operation structure\nRight structure: <set_name_1> <operator> <set_name_2> ")
            .also { return }

    if (data[1] !in OPERATORS) errorMessage("unknown operator \"${data[1]}\"").also { return }
...
fun makeInversionOfSet(name: String) {
    ...
    var flag = true
    for (operator in OPERATORS) if (operator in name) flag = false
